...
void esp_zb_app_signal_handler(esp_zb_app_signal_t *signal_struct)
{
    ...
    case ESP_ZB_BDB_SIGNAL_DEVICE_REBOOT:
        if (err_status == ESP_OK) {
            ...
            // Only start network steering if not already on network
            if (esp_zb_bdb_is_factory_new() || esp_zb_get_short_address() == 0xFFFE) {
                ESP_LOGI(TAG, "Start network steering");
                esp_zb_bdb_start_top_level_commissioning(ESP_ZB_BDB_MODE_NETWORK_STEERING);
            } else {
                ESP_LOGI(TAG, "Device already on network (Short Address: 0x%04hx)", esp_zb_get_short_address());
            }
        } else {
    ...
    if (message->info.dst_endpoint == HA_COLOR_DIMMABLE_LIGHT_ENDPOINT) {
        ...
        case ESP_ZB_ZCL_CLUSTER_ID_COLOR_CONTROL:
            if (message->attribute.id == ESP_ZB_ZCL_ATTR_COLOR_CONTROL_CURRENT_X_ID && message->attribute.data.type == ESP_ZB_ZCL_ATTR_TYPE_U16) {
                ...
                uint8_t saturation = *(uint8_t *)esp_zb_zcl_get_attribute(message->info.dst_endpoint, message->info.cluster,
                                                                          ESP_ZB_ZCL_CLUSTER_SERVER_ROLE, ESP_ZB_ZCL_ATTR_COLOR_CONTROL_CURRENT_SATURATION_ID)
                                         ->data_p;
                ESP_LOGI(TAG, "Light hue changes to %d, saturation %d", hue, saturation);
                light_driver_set_color_hue_sat(hue, saturation);
                ...
                uint8_t hue = *(uint8_t *)esp_zb_zcl_get_attribute(message->info.dst_endpoint, message->info.cluster,
                                                                   ESP_ZB_ZCL_CLUSTER_SERVER_ROLE, ESP_ZB_ZCL_ATTR_COLOR_CONTROL_CURRENT_HUE_ID)
                                  ->data_p;
                ESP_LOGI(TAG, "Light saturation changes to %d, hue %d", saturation, hue);
                light_driver_set_color_hue_sat(hue, saturation);
            } else if (message->attribute.id == ESP_ZB_ZCL_ATTR_COLOR_CONTROL_COLOR_MODE_ID) {
                uint8_t color_mode = message->attribute.data.value ? *(uint8_t *)message->attribute.data.value : 0;
                ESP_LOGI(TAG, "Color mode changed to %d (0=HS, 1=XY, 2=Temp)", color_mode);
            } else {
